# vercel-community/rust - Rust runtime for Vercel Functions
...
```mermaid
graph TD
    A["Function Invocation"] --> |"process_request(event: InvocationEvent) → Request"| B[Request]    B --> |"handler_fn(req: Request) → Future, Error>>"| C["Runtime calls handler_fn"]    C --> |"Ok(r) => process_response(r)"| D["Response"]
```
